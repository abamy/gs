...
const AUTHOR_DOMAIN = getMetadata('author');
const PUBLISH_DOMAIN = getMetadata('publish');
const DELIVERY_DOMAIN = getMetadata('delivery');
...
export async function fetchContentFragmentByPath(fragmentPath) {
  try {
    const apiUrl = isAuthorMode
      ...
      : `${PUBLISH_DOMAIN}/graphql/execute.json/gs/offerByPath;path=${fragmentPath}`;
